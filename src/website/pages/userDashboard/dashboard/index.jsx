...
import ChatboxSlider from "./ChatboxSlider.jsx";
import { useLoading } from "../../../../context/loading-context.js";
...
const UserDashboard = () => {
  ...
  const filteredOfferList = useSelector(
    (state) => state.offer.deviceFilterOfferList
  );
  ...
  const totalReferaEarning = useSelector(
    (state) => state?.referal?.referEarnings
  );
  ...
  const totalCompletedOffer = useSelector(
    (state) => state?.offer?.completedOffer
  );
  const { currencyValue } = useSelector((state) => state.user);
  ...
  const handleFilterData = (deviceName) => {
    ...
    const filteredData = offerList.filter((item) =>
      item.devices.toLowerCase().includes(deviceName.toLowerCase())
    );
    if (
      checkedDevices.android ||
      checkedDevices.ios ||
      checkedDevices.desktop
    ) {
  ...
  useEffect(() => {
    ...
    // Execute all async actions and wait for them to finish
    Promise.all([
      dispatch(fetchTotalReferl(auth.id)).unwrap(),
      dispatch(fetchTotalPoint(auth.id)).unwrap(),
      dispatch(fetchTotalAmount(auth.id)).unwrap(),
      dispatch(fetchOfferList(currentPage)).unwrap(),
      Promise.resolve(fetchOffer(currentPage)), // Ensure it's treated as a promise
      dispatch(fetchSurveyList()).unwrap(),
      dispatch(fetchCompletedOffer(auth.id)).unwrap(),
      dispatch(fetchTotalReferEarning(auth.id)).unwrap(),
      dispatch(fetchTotalEarning(auth.id)).unwrap(),
    ])
      .catch((error) => {
        console.error("Error fetching data:", error);
        toast.error(`Error fetching data: ${error.message}`);
      })
      .finally(() => stopLoading()); // Stop loader after everything is done
  }, []);
  

  // Fetch messages every 5 seconds
  useEffect(() => {
    dispatch(fetchUserLiveMessages()); // Initial fetch
    const interval = setInterval(() => dispatch(fetchUserLiveMessages()), 2000);
  ...
  return (
    <>
      <ChatboxSlider liveMessages={liveMessages} isUSDChecked={isUSDChecked} />
        ...
        <div className="dashboard-top flex justify-between items-center pb-2">
          <div>
            <div className="setDevice flex gap-4 items-center">
              <h5 className="md:text-xl text-xl text-white text-left md:font-medium font-medium">
                Device
              </h5>
          ...
          <div className="justify-end items-center gap-2 showUSD icon hidden custom-showUSD">
            <p>Show USD</p>
      ...
      <div className="total-earning md:p-4 p-3">
        <h5 className="md:text-2xl text-xl text-white text-left md:pb-6 pb-4 md:font-bold font-medium">
          Earnings
        </h5>
          ...
          <div className="earning-wrapper flex justify-between gap-4 flex-col md:w-1/2 w-full">
            <div className="flex gap-4 justify-between">
              <div className="item flex justify-start items-center md:gap-4 gap-2">
                ...
                <div className="content text-gray-200 text-left">
                  <h5 className="md:text-2xl text-xl font-bold text-white">
                    {isUSDChecked ? `$${totalEarning}` : totalEarning * 100}
                  </h5>
              ...
              <div className="item flex justify-start items-center  md:gap-4 gap-2">
                ...
                <div className="content text-gray-200 text-left">
                  <h5 className="md:text-2xl text-xl font-bold text-white">
                    {totalCompletedOffer ? totalCompletedOffer : `0`}
                  </h5>
            ...
            <div className="flex gap-4">
              ...
              <div className="item flex justify-start items-center  md:gap-4 gap-2">
                ...
                <div className="content text-gray-200 text-left">
                  <h5 className="md:text-2xl text-xl font-bold text-white">
                    {totalReferaEarning ? `$${totalReferaEarning}` : `$0`}
                  </h5>
            ...
            <div className="refer-user bg-white rounded-md p-4 text-left">
              ...
              <div>
                ...
                <p className="text-gray-200">
                  Share your referral link to your friends, and get 10% of their
                  lifetime in rewards.
                </p>
                <img src={referimg} alt="referimg" />
              </div>
              <Link
                to={`/subuser/signup/${referralCode}`}
                className="cursor-pointer"
                target="_blank"
              >
                <input
                  className="cursor-pointer"
                  value={`https://coinlooty.com/ref/${referralCode}`}
                  readOnly
                />
              </Link>
            </div>
      ...
      <div className="offers-box md:p-6 p-4">
        <div className="sec-topHeading">
          <h5 className="md:text-2xl text-xl text-white text-left md:font-bold font-medium">
            Featured Offers
          </h5>
          <Link to="/alloffers" className="text-white text-sm font-medium">
            View All
          </Link>
        </div>
        <Swiper
          ...
          navigation={true}
          modules={[Navigation]}
          className="mySwiper items-wrapper flex gap-4"
        >
      ...
      <div className="survey-box md:p-6 p-4">
        <div className="sec-topHeading">
          ...
          <Link to="/allsurveys" className="text-white text-sm font-medium">
            View All
          </Link>
        </div>
        <Swiper
      ...
      <div className="md:p-6 p-4 offerWall">
        <div className="survey-partner-box">
          ...
          <Swiper
            ...
            navigation={true}
            modules={[Navigation]}
            className="partnerSwiper spItem-wrapper flex gap-4"
          >
            {parnerData?.partners.map((item, index) => (
              <SwiperSlide key={item?.id}>
                <div className="item">
                  <div
                    className="spItem flex justify-between items-center gap-4 flex-col"
                    style={{
                      background:
